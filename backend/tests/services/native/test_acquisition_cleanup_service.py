...
@pytest.mark.asyncio
async def test_torrent_cleanup_never_unlinks_a_seeding_torrents_files(tmp_path: Path):
    """A completed torrent keeps seeding: the import COPIED its bytes out, so
    cleanup must discard client artifacts (a no-op retain for qBittorrent) and
    leave every local file in place - never fall through to the slskd unlink."""
    root = tmp_path / "qbittorrent"
    album = root / "seeding-album"
    album.mkdir(parents=True)
    seeding = album / "track.flac"
    seeding.write_bytes(b"seeding")
    store = _store(tmp_path)
    attempt = await _attempt(store, root, source="torrent", paths=[seeding])
    client = _Client(
        DownloadMaterialization(
            state="completed",
            mount_root=str(root),
            workspace_path=str(album),
            # The torrent client reports no attempt-owned file_paths on purpose.
            file_paths=[],
            mount_healthy=True,
        )
    )
    ...
    assert seeding.read_bytes() == b"seeding"
    assert album.is_dir()
    assert client.discarded == 1
