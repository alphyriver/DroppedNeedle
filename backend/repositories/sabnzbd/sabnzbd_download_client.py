...
class SabnzbdDownloadClient:
    ...
    def _matches(slot_id: str, slot_name: str, handle: TaskHandle) -> bool:
        # A KNOWN external id is authoritative: never fall back to the correlation
        # key, or a second job reusing the same name would match this handle.
        if handle.external_id:
            return slot_id == handle.external_id
