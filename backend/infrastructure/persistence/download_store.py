...
CREATE TABLE IF NOT EXISTS download_attempts (
    ...
    source TEXT NOT NULL CHECK(source IN ('soulseek','usenet','torrent')),
...
class DownloadStore(PersistenceBase):
    ...
    def _ensure_tables(self) -> None:
        ...
        try:
            ...
            self._migrate_download_attempt_sources(conn)
    ...
    def _migrate_download_attempt_sources(self, conn: sqlite3.Connection) -> None:
        """Widen ``download_attempts.source`` to admit ``torrent``.

        The table shipped with ``CHECK(source IN ('soulseek','usenet'))``, which
        rejects every torrent attempt and so blocks the whole cleanup journal for
        that source. SQLite cannot ALTER a CHECK, so an existing table carrying the
        narrow constraint is rebuilt in place - same approach as
        ``_migrate_quarantine``. Indexes are schema-global and follow the renamed
        table, so they are dropped before the new DDL recreates them."""
        row = conn.execute(
            "SELECT sql FROM sqlite_master WHERE type='table' AND name='download_attempts'"
        ).fetchone()
        if row is None or "'torrent'" in (row["sql"] or ""):
            return
        conn.execute("ALTER TABLE download_attempts RENAME TO download_attempts_legacy")
        for index in (
            "idx_download_attempts_task",
            "idx_download_attempts_cleanup",
            "idx_download_attempts_job",
        ):
        ...
        columns = ", ".join(
            ...
            for row in conn.execute(
                "PRAGMA table_info(download_attempts_legacy)"
            ).fetchall()
        )
        # Column-explicit copy: the legacy table may predate later ADD COLUMN
        # migrations, so the new table's defaults must fill the gaps.
        conn.execute(
            f"INSERT INTO download_attempts ({columns}) "  # noqa: S608 - names from PRAGMA
            f"SELECT {columns} FROM download_attempts_legacy"
        )
        conn.execute("DROP TABLE download_attempts_legacy")
